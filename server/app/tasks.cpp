...
/**
 * @file tasks.cpp
 * @brief Логика проверки состояния задач и их математическая/алгоритмическая генерация.
 */

/**
 * @details Проверяет валидность задачи на основе её типа (тип 0 считается неинициализированным).
 */
...
/**
 * @details Сбрасывает поля структуры задачи в исходные значения.
 */
...
/**
 * @details Генерирует уникальный вариант задачи в зависимости от переданного числового типа (1..5).
 * Наполняет payload параметрами, преобразует правильный математический ответ в строку с нужной точностью.
 * - Задача 1: Формула численного интегрирования Симпсона.
 * - Задача 2: Метод хорд для нелинейных уравнений.
 * - Задача 3: Метод дихотомии (деления отрезка пополам).
 * - Задача 4: Поиск листьев графа (дерева) с помощью алгоритма обхода в глубину (DFS).
 * - Задача 5: Вычисление выражения с аналитической функцией (логарифмы/корни).
 */
