...
bool Database::is_reg_ok(QString login, QString email, QString password, int socket_descriptor)
{
    ...
    query.prepare("INSERT INTO User(login, email, password, socket_id, role, task1_stat, task2_stat, task3_stat, task4_stat)"
                  "VALUES (:login, :email, :password, :socket_id, \'user\', 0, 0, 0, 0)");
    query.bindValue(":login", login);
    query.bindValue(":email", email);
    query.bindValue(":password", password);
    query.bindValue(":socket_id", socket_descriptor);
...
{
    QSqlQuery query(db);
    query.prepare("SELECT task1_stat, task2_stat, task3_stat, task4_stat from User where socket_id = :socket_id");
    query.bindValue(":socket_id", socket_descriptor);
    if (!query.exec()) {
    ...
    if (query.next()) {
        ...
        return query.value(task1_stat_index).toString() + "||" + query.value(task2_stat_index).toString()
               + "||" + query.value(task3_stat_index).toString() + "||" + query.value(task4_stat_index).toString() + "\r\n";
    }
...
QString Database::get_all_stat(int socket_descriptor)
{
    ...
    if (is_admin(socket_descriptor)) {
        if (!query.exec("SELECT login, task1_stat, task2_stat, task3_stat, task4_stat from User")) {
            return query.lastError().text();
        }
